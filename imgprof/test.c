...
void linea(int p1x,int p1y,int p2x, int p2y, const vector<int> &punti_src){
  ...
  for (int s=0;s<steps;s++) {
    ...
    r=punti_src[x*src.width()*3 + y*3];
    g=punti_src[x*src.width()*3 + y*3 + 1];
    b=punti_src[x*src.width()*3 + y*3 + 2];

    r1=punti_src[x1*src.width()*3 + y1*3];
    g1=punti_src[x1*src.width()*3 + y1*3 + 1];
    b1=punti_src[x1*src.width()*3 + y1*3 + 2];
...
int analisi_immagine(const char* path) {
  ...
  //creo io il vettore di punti al posto di usare l'immagine così lo posso passare al device
  vector<int> punti_src;
  for(int x=0; x<src.width(); ++x) {
    for(int y=0; y<src.height(); ++y) {
      punti_src.push_back((int)src(x,y,0,0));
      punti_src.push_back((int)src(x,y,0,1));
      punti_src.push_back((int)src(x,y,0,2));
    }
  }

  int p1x[th],p1y[th],p2x[th],p2y[th];
  ...
  for (int q=0;q<th;q+=step){
    linea(p1x[q],p1y[q],p2x[q],p2y[q],punti_src);
  }
  ...
  int pp1x[th],pp1y[th],pp2x[th],pp2y[th];
  for (int q=0;q<th;q+=step){
    pp1x[q]=0;
    pp1y[q]=sy*q/th;
    pp2x[q]=sx-1;
  ...
  for (int q=0;q<th;q+=step){
    linea(pp1x[q],pp1y[q],pp2x[q],pp2y[q],punti_src);
  }
